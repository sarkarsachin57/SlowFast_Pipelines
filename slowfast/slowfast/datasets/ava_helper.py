...
AVA_VALID_FRAMES = range(-1, 10000000)
...
def load_boxes_and_labels(cfg, mode):
    ...
    print(f"ann_filenames : {ann_filenames}")
    print(f"ann_is_gt_box : {ann_is_gt_box}")
    print(f"detect_thresh : {detect_thresh}")
    print(f"boxes_sample_rate : {boxes_sample_rate}")
    ...
    for video_idx in range(len(boxes_and_labels)):
        ...
        for sec in boxes_and_labels[video_idx].keys():
            ...
            if len(boxes_and_labels[video_idx][sec]) > 0:
                ...
                n_no_labels_in_sec += 1

    print(f"count : {count}")
    print(f"n_cnt : {n_cnt}")
    print(f"n_not_valid_frame_filtered : {n_not_valid_frame_filtered}")
    print(f"n_no_labels_in_sec : {n_no_labels_in_sec}")
...
def parse_bboxes_file(ann_filenames, ann_is_gt_box, detect_thresh, boxes_sample_rate=1):
    ...
    for filename, is_gt_box in zip(ann_filenames, ann_is_gt_box):
        with pathmgr.open(filename, "r") as f:
            for line in f:
                ...
                # print(f"all_boxes[video_name] : {all_boxes[video_name]}")
    ...
    print(f"count : {count}")
    print(f"n_det_thresh_filtered : {n_det_thresh_filtered}")
    print(f"n_sample_rate_filtered : {n_sample_rate_filtered}")
