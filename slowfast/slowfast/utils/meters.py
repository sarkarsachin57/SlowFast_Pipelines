...
class AVAMeter:
    ...
    def update_stats(self, preds, ori_boxes, metadata, loss=None, lr=None):
        """
        Update the current stats with shape validation.
        """
        if self.mode in ["val", "test"]:
            if ori_boxes.shape[1] != 5:
                return
            # Check if we have existing boxes to compare shape with
            if self.all_ori_boxes and ori_boxes.shape[1] != self.all_ori_boxes[0].shape[1]:
                # Log warning and handle shape mismatch
                logger.warning(f"Box dimension mismatch: {ori_boxes.shape[1]} vs {self.all_ori_boxes[0].shape[1]}")
                
                # # Option 1: Pad the smaller dimension
                # if ori_boxes.shape[1] < self.all_ori_boxes[0].shape[1]:
                #     padding = torch.zeros((ori_boxes.shape[0], self.all_ori_boxes[0].shape[1] - ori_boxes.shape[1]), 
                #                         device=ori_boxes.device)
                #     ori_boxes = torch.cat([ori_boxes, padding], dim=1)
                # # Option 2: Truncate the larger dimension
                # else:
                #     ori_boxes = ori_boxes[:, :self.all_ori_boxes[0].shape[1]]
